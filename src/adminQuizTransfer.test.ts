...
import { clearHelper, adminQuizRestoreHelper, adminAuthRegisterHelper, adminQuizListHelper, adminQuizCreateHelper, adminQuizRemoveHelper, adminQuizTransferHelper } from "./httpHelperFunctions";
...
describe("adminQuizTransfer", () => {
  ...
  beforeEach(() => {
    ...
    const registerResponse2 = adminAuthRegisterHelper("user2@tookah.com", "Passwordx123", "John", "Davies");
  ...
  describe("Testing failure cases", () => {
    test("Recipient's email is invalid", () => {
      ...
      if("quizId" in quizId) {
        expect(adminQuizTransferHelper(sessionId, quizId.quizId, "user4@tookah.com")).toStrictEqual({
    ...
    test("Recipient email is the currently logged in user", () => {
      const quizId = adminQuizCreateHelper(sessionId, "Kelly", "Kelly Kills Keys");
      if ("quizId" in quizId) {
        expect(adminQuizTransferHelper(sessionId, quizId.quizId, "user1@tookah.com")).toStrictEqual({
          error: expect.any(String),
        });
      }
    });
    test("Invalid sessionId", () => {
      const quizId = adminQuizCreateHelper(sessionId, "Kelly", "Kelly Kills Keys");
      if("quizId" in quizId) {
        expect(adminQuizTransferHelper(sessionId + 5, quizId.quizId, "user2@tookah.com")).toStrictEqual({
          error: expect.any(String)
        });
      }
    });
    test("Valid token, but is not owner of quiz", () => {
      const quizId = adminQuizCreateHelper(sessionId, "Kelly", "Kelly Kills Keys");
      const quizId2 = adminQuizCreateHelper(sessionId2, "Batmobile", "Batman will jump");
      if("quizId" in quizId && "quizId" in quizId2) {
        expect(adminQuizTransferHelper(sessionId, quizId2.quizId, "user2@tookah.com")).toStrictEqual({
          error: expect.any(String),
        });
      }
    });
    test("Valid token, but quiz does not exist", () => {
      const quizId = adminQuizCreateHelper(sessionId, "Kelly", "Kelly Kills Keys");
      if("quizId" in quizId) {
        expect(adminQuizTransferHelper(sessionId, quizId.quizId + 5, "user2@tookah.com")).toStrictEqual({
          error: expect.any(String),
        });
      }
    });
    test("QuizId name is already being used by target user", () => {
      const quizId = adminQuizCreateHelper(sessionId, "Kelly", "Kelly Kills Keys");
      const quizId2 = adminQuizCreateHelper(sessionId2, "Kelly", "Batman will jump");
      if("quizId" in quizId && "quizId" in quizId2) {
