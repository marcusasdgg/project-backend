...
import { clearHelper, adminQuizNameUpdateHelper, adminAuthRegisterHelper, adminQuizInfoHelper, adminQuizCreateHelper, adminQuizAddQuestionHelper, adminQuizQuestionUpdateHelper } from "./httpHelperFunctions";
...
describe("AdminQuizQuestionUpdate", () => {
        ...
        test("Normal usage: update question of a singular quiz with single question created by user", () => {
            ...
            if ("sessionId" in token){
                ...
                if ("quizId" in quiz){
                    ...
                    let question = adminQuizAddQuestionHelper(token.sessionId, quiz.quizId,questionBody)
                    if ("questionId" in question){
                        questionBody.duration = 1;
                        expect(adminQuizQuestionUpdateHelper(quiz.quizId, question.questionId, token.sessionId, questionBody)).toStrictEqual({});
                    }
                }
        ...
        test("Normal usage: update question of a singular quiz with multiple questions created by user", () => {
            ...
            if ("sessionId" in token){
                ...
                if ("quizId" in quiz){
                    ...
                    let questionbqody2 : QuestionBody =  {
                    ...
                    let question = adminQuizAddQuestionHelper(token.sessionId, quiz.quizId,questionBody)
                    let question2 = adminQuizAddQuestionHelper(token.sessionId, quiz.quizId,questionbqody2)
                    if ("questionId" in question){
                        questionBody.duration = 1;
                        expect(adminQuizQuestionUpdateHelper(quiz.quizId, question.questionId, token.sessionId, questionBody)).toStrictEqual({});
                    }
    ...
    describe("Failure Cases", () => {
        ...
        test("Question Id does not refer to a valid question within any quiz", () => {
            expect(adminQuizQuestionUpdateHelper(quizId, invalidQuestionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
        });
        

        test("Question ID refers to a valid question within another quiz but not referenced one", () => {
            let quiz2 = adminQuizCreateHelper(token, "test quiz 2", "description");
            if ("quizId" in quiz2){
                let questionId2 = adminQuizAddQuestionHelper(token, quiz2.quizId, questionBody);
                if ("questionId" in questionId2){
                    expect(adminQuizQuestionUpdateHelper(quizId, questionId2.questionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
                } 
            }
        });

        test("Token does not refer to a valid user", () => {
            expect(adminQuizQuestionUpdateHelper(quizId, questionId, invalidToken, questionBody)).toStrictEqual({error: expect.any(String)});
        });
        ...
        test("Token provided refers to a user who doesnt own the quiz", () => {
            ...
            if ("sessionId" in user2) {
                expect(adminQuizQuestionUpdateHelper(quizId, questionId, user2.sessionId, questionBody)).toStrictEqual({error: expect.any(String)});
            }
            
        });

        test("Token provided refers to a user, and quiz doesnt exist", () => {
            expect(adminQuizQuestionUpdateHelper(invalidQuizId, questionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
        });
        ...
        test("Question string is less than 5 characters in length or greater than 50 characters in length", () => {
            ...
            expect(adminQuizQuestionUpdateHelper(quizId, questionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
            questionBody.question = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
            expect(adminQuizQuestionUpdateHelper(quizId, questionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
        });
        ...
        test("The question has more than 6 answers or less than 2 answers", () => {
            ...
            expect(adminQuizQuestionUpdateHelper(quizId, questionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
            questionBody.answers = [{answer: "abc", correct: true},{answer: "def", correct: false},{answer: "zzz", correct: false},{answer: "tef", correct: false},{answer: "mam", correct: false},{answer: "des", correct: false},{answer: "sss", correct: false}];
            expect(adminQuizQuestionUpdateHelper(quizId, questionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
        });
        ...
        test("The length of any answer is shorter than 1 character long, or longer than 30 characters long", () => {
            ...
            expect(adminQuizQuestionUpdateHelper(quizId, questionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
            questionBody.answers[0].answer = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
            expect(adminQuizQuestionUpdateHelper(quizId, questionId, token, questionBody)).toStrictEqual({error: expect.any(String)});
        });
