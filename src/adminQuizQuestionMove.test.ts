...
describe('V2', () => {
  ...
  describe('Success Cases', () => {
    test('should move a question within a quiz for a valid user', () => {
      const registerResponse: error | sessionIdToken =
        adminAuthRegisterHelper('validemail21@gmail.com', '123abc!@#', 'John', 'Doe');
      expect(registerResponse).toStrictEqual({
        sessionId: expect.any(Number),
      });
  ...
  describe('Failure Cases', () => {
    test('should return an error if token is invalid', () => {
      const registerResponse: error | sessionIdToken =
        adminAuthRegisterHelper('validemail22@gmail.com', '123abc!@#', 'Jane', 'Doe');
      expect(registerResponse).toStrictEqual({
        sessionId: expect.any(Number),
      });
      ...
      if ('sessionId' in registerResponse) {
        ...
        const moveQuestionResponse = adminQuizQuestionMoveV2Helper(invalidToken, quizId, questionId, newPosition);
        expect(moveQuestionResponse).toStrictEqual({
          error: 'invalid Token',
        });
      }
    ...
    test('should return an error if quizId or questionId is invalid', () => {
      const registerResponse: error | sessionIdToken =
        adminAuthRegisterHelper('validemail23@gmail.com', '123abc!@#', 'Mark', 'Smith');
      expect(registerResponse).toStrictEqual({
        sessionId: expect.any(Number),
      });
    ...
    test('should return an error if newPosition is out of range', () => {
      const registerResponse: error | sessionIdToken =
        adminAuthRegisterHelper('validemail23@gmail.com', '123abc!@#', 'Emily', 'Johnson');
      expect(registerResponse).toStrictEqual({
        sessionId: expect.any(Number),
      });
      ...
      if ('sessionId' in registerResponse) {
        ...
        if ('quizId' in createQuizResponse) {
          ...
          if ('questionId' in addQuestionResponse) {
            ...
            const moveQuestionResponse = adminQuizQuestionMoveV2Helper(validToken, quizId, questionId, invalidNewPosition);
            expect(moveQuestionResponse).toStrictEqual({
              error: 'New position is out of range',
            });
          }
